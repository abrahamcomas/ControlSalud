 <div class="row"> 
	...
	<div class="col-xs-12 col-sm-12 col-md-12 col-lg-4">
	  	<br> 
  		<center>  
    		<h3><strong>EDITAR ALUMNOS {{ $ID_Alumno_S }}</strong></h3>	
		</center> 
      	<hr> 
    	...
    	<hr>
    	<div class="form-label-group">
			<center><h4>SELECCIONAR ALUMNO DE <strong>({{ $NombreCursoSel }})</strong></h4></center>
      		<select class="form-control" size="4" wire:model="ID_Alumno_S">
        		@foreach ($ListaAlumnos as $row)
          			<option value="{{ $row->id_Alumno }}">
          				{{ $row->Nombres  }} <strong>({{ $row->Apellidos }})</strong>
          			</option>
        		@endforeach
      		</select> 
      		<hr>
          	<center>
            	<button type="button" class="btn btn-info active" wire:click="AlumnoSeleccionado">
               		seleccionar
            	</button>
          	</center>  
    	</div>
		...
		<div class="panel-body"> 
			...
			@if($Editar=='1') 
			    ...
			    <div class="form-group">
		       	...
		       	<hr> 
	        	<center>
	   				<button type="button" class="btn btn-info active" wire:click="UpdateAlumno">Editar</button>
	  			</center> 
			@endif
			<center>
	        	<strong>
